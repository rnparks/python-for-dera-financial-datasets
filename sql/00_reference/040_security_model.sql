...
CREATE TABLE sec_reference.security (
    ...
    -- basis is recorded because the three sources are not equally strong:
...
CREATE TABLE sec_reference.delisting_event (
    ...
    reason           TEXT   NOT NULL CHECK (reason IN
                        ('exchange_notice','acquisition','bankruptcy',
                         'merger','liquidation','unknown')),
    is_provisional   BOOLEAN NOT NULL DEFAULT FALSE,
    source_form      TEXT,
    source_adsh      TEXT,
...
COMMENT ON COLUMN sec_reference.security.first_trade_basis IS
    'Evidence class: 8-A is a listing registration, 424B a pricing, and '
    'first_edgar_filing only a floor on company existence -- not a '
    'trading date, and flagged so callers can exclude it.';
