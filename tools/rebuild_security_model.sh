...
psql "$PG_DSN" -v ON_ERROR_STOP=1 -q -f sql/06_security/010_security_populate.sql
psql "$PG_DSN" -v ON_ERROR_STOP=1 -q -f sql/06_security/020_eligibility.sql 2>&1 | grep -v NOTICE || true
