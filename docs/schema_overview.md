...
| **`index_membership`** | **6,334** | **Dated membership with GICS as of the interval, all three indexes replayed**: S&P 500 1,860 intervals over 840 companies, S&P 400 2,151 over 1,045, S&P 600 2,323 over 1,160; 32 intervals re-keyed to a predecessor registrant by `cik_succession`; no snapshot rows remain |
| `index_membership_latest` | 2,416 | One row per company: its current or most recent membership, the label gold falls back to |
| `index_membership_timeline` | 6,344 | `index_membership` resolved to one non-overlapping interval set per company (2,416 companies; the later start wins). What gold joins with a plain range condition; an EXCLUDE constraint enforces the non-overlap |
