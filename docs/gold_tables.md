...
- **Index membership is dated for all three indexes, with one known hole.** The two `tradable_financials` matviews and `peer_stats` draw their population from `sec_reference.index_membership`: replayed monthly Wikipedia history for the S&P 500 (822 companies since 2008), the S&P 400 (1,027 since 2011) and the S&P 600 (1,157 since 2018). Through the S&P 600's 2019-12 to 2021-02 gap, where its page and the S&P 400's disagreed, only the 444 members seen on both sides are present; `data_sources.md` explains. 138 early tickers are unresolved and listed. A company that re-registered under a new CIK is split at the handoff (`sec_reference.cik_succession`), so a FY2019 S&P 500 panel scores Apache under 6769 and a FY2022 one under APA Corp 1841666. `fact_asof`, `share_class_shares` and everything in `sec_reference` cover the full spine regardless.
