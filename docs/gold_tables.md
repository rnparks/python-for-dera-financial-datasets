...
**Data coverage** (as of the 2026q2 load, figures dated 2026-09-11): filings from 2009-04-15 through 2026-06-30, 16.2M rows per display matview (2,400 companies that have ever been index constituents) and 97.9M in `fact_asof`.
...
2. Find the newest period at which the **formula** resolves — candidate periods are every period at which any operand exists, newest first, up to twelve, and the formula is evaluated at each until one succeeds. A period missing a required operand falls through to the next rather than ending the search. A formula has **variants** (`concept_formula_variant`), tried in order at each period: a variant applies only within its industries, and yields nothing when its **guard** matches a custom-namespace balance tag the filer reports at that date.
3. **The newer period wins.** At the same period a filed figure beats a reconstruction.
4. **A zero comes only from the balance sheet itself** (`total_debt`). When neither a tag nor a formula produced a figure, the newest annual filing's balance-sheet face is read (`debt_face`): if it is complete, prints no borrowing line with a value, carries no borrowing line without an undimensioned value, its interest expense is nil and the filer is not a bank, total debt is zero for that filing. A zero fills a NULL and never replaces a figure, and it is never inherited: only the newest filing's face counts, so a company that has borrowed since its debt-free years shows what its latest face shows.
...
| `index_name` | text | Index the company belonged to **as of `tradable_from`**, from replayed history for all three indexes (S&P 500 since 2008, S&P 400 since 2011, S&P 600 since 2018) |
...
**The balance sheet as the filer printed it, read for borrowings.** One row per 10-K and 10-Q (401,089 filings, 2026-09-11): the liabilities section of the balance-sheet face from `sec_silver.pre_silver`, each line judged by its printed label (debt, borrowings, notes payable, line of credit, term loan, senior and convertible notes, subordinated, mortgages payable, repurchase agreements, FHLB advances, commercial paper, financing obligations; not receivables, deposits, deferred items or leases unless the line also says debt), the lines' undimensioned values, the filing's annualised interest expense and its total assets.

`zero_by_face` is true for 82,342 filings: the face is complete, no borrowing line carries a positive value, none lacks an undimensioned value, interest is at most 0.2% of assets, and the filer is not a bank or broker. `reason` records why every other face is not a zero ("borrowing line present", "borrowing line without an undimensioned value", "interest expense without a borrowing line", "bank or broker: not judged", "face incomplete"). `borrowing_labels` lists the lines found, so a zero or a refusal can be checked against the filing in seconds.

Why it exists: a company with no debt files no debt tag, so the tag walk resolves its `total_debt` to NULL and every leverage cross-section silently drops the least-levered names. A zero cannot come from a missing tag, but it can come from the statement. Measured 2026-09-11 against 135 S&P 400/600 members verified two ways (every DERA fact under any namespace or dimension; an external balance sheet with leases separated): the rule gave 90 zeros among the 109 verified debt-free and none among the 23 with debt. It fills NULLs only: against the 1,202 members whose FY2024 total debt already resolved from tags, no value moved. A borrowing line is read here, never summed: a face with a borrowing line the tag walk cannot value stays NULL and is listed by `reason`.
...
| `debt_secured`, `debt_unsecured`, `debt_senior_notes`, `debt_line_of_credit`, `debt_notes_payable`, `debt_loans_payable`, `debt_convertible`, `debt_other_long_term`, `debt_subordinated`, `debt_junior_subordinated`, `debt_fhlb_advances`, `debt_other_borrowings`, `debt_short_term_borrowings`, `debt_warehouse`, `debt_surplus_notes`, and the `_current` split forms of secured, line of credit, loans payable and FHLB advances | Instrument lines | balance | USD | The balance-sheet lines a bank, REIT or insurer files instead of a split; operands of `total_debt`'s second variant. **Not scored**: absent from `peer_stats` and the snapshots (`canonical_concepts.scored`) |
...
| Industry variants | `RevenuesNetOfInterestExpense`, `InterestAndDividendIncomeOperating` | Mapped under a `sic_prefix` so they outrank the generic rows only for that industry — which is why every industry block restates the generic totals above its own line: a broker filing `Revenues` beside `InvestmentBankingRevenue` keeps the total (Stifel), an advisory boutique filing only the component gets it (Moelis) |
| Instrument lines | `SecuredDebt`, `AdvancesFromFederalHomeLoanBanks`, `SubordinatedDebt` | One concept per balance-sheet line, each including both portions; summed by `total_debt`'s second variant for banks, insurers and REITs only, and only when no custom-namespace debt line is on the balance sheet at that date (see Derived concepts) |
| Units | `DebtAndCapitalLeaseObligations` in EUR, GBP, JPY | Every tag walk takes dollar facts only (`uom = 'USD'`; DERA records per-share values in USD too). Berkshire files its foreign-currency notes as undimensioned rows in those currencies, and until 2026-09-11 `total_debt` resolved to whichever the tie-break picked — 1.26 trillion yen for FY2023, read as dollars |
...
- **Index membership is dated for all three indexes, with one known hole.** The two `tradable_financials` matviews and `peer_stats` draw their population from `sec_reference.index_membership`: replayed monthly Wikipedia history for the S&P 500 (841 companies since 2008), the S&P 400 (1,046 since 2011) and the S&P 600 (1,160 since 2018). Through the S&P 600's 2019-12 to 2021-02 gap, where its page and the S&P 400's disagreed, only the 444 members seen on both sides are present; `data_sources.md` explains. 36 tickers are unresolved and listed, none of them a company with facts in DERA. A company that re-registered under a new CIK is split at the handoff (`sec_reference.cik_succession`), or at the successor's first EDGAR filing where SEC's file dates the handoff late, so a FY2019 S&P 500 panel scores Apache under 6769 and a FY2022 one under APA Corp 1841666, a FY2012 panel Google Inc and a FY2016 one Alphabet, and Exxon Mobil's 34088 holds the seat until ExxonMobil Holdings took XOM on 2026-07-16. `fact_asof`, `share_class_shares` and everything in `sec_reference` cover the full spine regardless.
...
- **`total_debt` is a roll-up.** Four total tags resolve it directly; otherwise `concept_formula` sums `debt_noncurrent + debt_current`, and the noncurrent operand is **required**: with both optional, 26 FY2024 totals were the current portion alone (Deere 15.9B against roughly 65B), so those now resolve to nothing rather than to a wrong number. `LongTermDebtNoncurrent` was once priority 3 on `total_debt` itself and understated it the same way. Where a bank, insurer or REIT files no split at all, a second variant sums its instrument lines (Acadia Realty: mortgages 954M, unsecured notes 570M, revolver 14M), unless a custom-namespace debt line is on the balance sheet, in which case nothing (2026-09-11: FY2024 total debt for 325 of the 400 S&P 400 members, from 299, and 452 of the 600 S&P 600 members, from 397; 148 members file no debt line at all and are not asserted to be debt-free). Not mapped on purpose: instrument lines of a non-financial, and anything dimensioned. A company that resolves nothing by tag or formula reads zero only when its newest annual balance-sheet face shows no borrowing line and its interest is nil (`debt_face`; 145 FY2024 members, from 2026-09-11).
...
- `--refresh-only` refreshes all six matviews in dependency order, `peer_stats` last.
...
| `total_debt`, variant 1 | `debt_noncurrent + debt_current` | Fires only when no combined debt tag resolves |
| `total_debt`, variant 2 | the sum of the instrument lines: secured, unsecured, senior notes, line of credit, notes and loans payable, convertible, other long-term, subordinated, junior subordinated, FHLB advances, other and short-term borrowings, warehouse, surplus notes | A bank (SIC 60), an insurer (63) or a REIT (6798) files no split, only lines; every line optional, at least one present. Fires only where variant 1 cannot, and not when the filer reports a custom-namespace debt line at that date |
...
**Variants are tried in order, scoped and guarded.** `concept_formula_variant` gives each variant an industry scope (`sic_prefixes`) and a guard: a regex over the filer's undimensioned balance-sheet tags in a custom namespace at that date (`guard_match`, less `guard_except`), and when one matches the variant yields nothing. The instrument sum is scoped and guarded for measured reasons (2026-09-11, every FY2024 10-K): the lines are disjoint — secured beside unsecured in 43 filers, line of credit beside secured in 43, FHLB advances beside subordinated in 32, never the same value — so the sum is the balance sheet's own arithmetic; but 51 of the 212 banks and 39 of the 129 REITs whose lines would be summed also carry a company-extension debt line the sum would miss (Home Bancshares' custom FHLB-and-other line, 601M beside 439M of subordinated debt in us-gaap; Omega Healthcare's custom senior notes beside 243M of secured debt; CNO's variable-interest-entity borrowings), and those resolve to nothing; and for non-financials 98 of the 259 sums with an interest-expense line implied a rate over 15%, the lines being partial (a revolver in us-gaap, the term loan in a custom tag), so the variant does not apply to them at all. A filed total still beats every variant at the same period. The guard exempts the premium and discount elements themselves but not a line named after one: Brixmor files its 5.3B of mortgages as `MortgagesPayableIncludingUnamortizedPremium`, and a blanket exclusion of those words had let that kind of line past (tightened 2026-09-11; no member of the 2024-12-31 panel changes).
...
| `037_debt_face.sql` | `debt_face` |
| `035_fact_asof.sql` | `fact_asof` |
| `040_helper_functions.sql` | `get_pit_financials()`, `get_financials_by_ticker()` |
| `050_canonical_concepts.sql` | `canonical_concepts`, `concept_tag_map`, `concept_formula_variant`, `concept_formula`, `concept_ratio` |
