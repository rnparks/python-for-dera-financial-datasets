...
uv run dera verify         # 60 data-correctness checks; non-zero exit on failure
...
One is kept by hand: `data/reference/index_cik_overrides.csv`, the constituent
runs no rule resolves, each row citing its evidence.
