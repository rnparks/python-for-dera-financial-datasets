...
- **52-check verification suite** in `tools/verify_pit.sql`, run by `dera verify` (~90 s); checks 29–52 each name the 2026-09-04 review defect, or the gap closed since, that they guard. Plus 102 unit tests (`uv run pytest`, no database) and GitHub Actions running `ruff` and `pytest` on every push.
...
- **`peer_stats` balance concepts are the latest instant inside the fiscal-year window, not the fiscal year-end.** 86% of FY2024 balance rows (6,324 of 7,378) are dated January–March 2025: a Q1 10-Q balance, because `fiscal_year_of` maps Jan–Mar period ends to the prior year and 080 takes the latest date. A FY2024 leverage ratio therefore divides December flows by March balances. Fix: assign the fiscal year from the filing's own `fy`/`fp`, or take the balance dated at the annual period end. Found 2026-09-04 while closing the debt gap; next on the list.
...
- (this commit) — feat(gold): close the S&P 500 total_debt and revenue gaps with 24 us-gaap tags; require the noncurrent operand so a total is never the current portion alone
