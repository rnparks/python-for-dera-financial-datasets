...
- **61-check verification suite** in `tools/verify_pit.sql`, run by `dera verify` (~90 s); checks 29–61 each name the 2026-09-04 review defect, or the gap closed since, that they guard. Plus 112 unit tests (`uv run pytest`, no database) and GitHub Actions running `ruff` and `pytest` on every push.
...
- (this commit) — feat(reference): constituent runs resolve by a dated name, a cited allowlist and the neighbouring run across partial captures; 138 unresolved tickers become 36
