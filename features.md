...
- **62-check verification suite** in `tools/verify_pit.sql`, run by `dera verify` (~90 s); checks 29–62 each name the 2026-09-04 review defect, or the gap closed since, that they guard. Plus 112 unit tests (`uv run pytest`, no database) and GitHub Actions running `ruff` and `pytest` on every push.
...
- **Debt coverage, the remainder.** Verified 2026-09-11, two sources each (every DERA fact under any namespace or dimension plus interest and debt cash flows; the FY2024 balance sheet on stockanalysis.com with leases separated): of the 135 S&P 400/600 members of 2024-12-31 with no plain debt line, 109 are debt-free on both, 1 carries finance leases only, 2 an immaterial loan, and 23 have debt the plain us-gaap lines never carry — custom tags (Brixmor, Community Financial), dimensioned-only facts (Cinemark, Hudson Pacific, Kennedy-Wilson) and mortgage-REIT repurchase agreements (Annaly, Armour, Ellington). A zero from absence is an inference the layer has never made, so `total_debt` stays NULL for the 109 until that call is made; the honest mechanism would be a cited per-company allowlist, like the share-class and index files. 26 members are refused by the custom-line guard, and about 60 non-financials file instrument lines the variant does not apply to. Brokers and lenders (SIC 61–62) are excluded from the sum: a third carry custom lines.
...
- (this commit) — fix(gold): the instrument-sum guard no longer exempts a custom debt line named after its premium or discount; the no-debt-line members verified against two sources
