...
- **51-check verification suite** in `tools/verify_pit.sql`, run by `dera verify` (~90 s); checks 29–51 each name the 2026-09-04 review defect, or the gap closed since, that they guard. Plus 102 unit tests (`uv run pytest`, no database) and GitHub Actions running `ruff` and `pytest` on every push.
...
- (this commit) — perf(gold): a non-overlapping membership timeline replaces the per-fact LATERAL in the three membership-reading matviews
