...
- **60-check verification suite** in `tools/verify_pit.sql`, run by `dera verify` (~90 s); checks 29–60 each name the 2026-09-04 review defect, or the gap closed since, that they guard. Plus 102 unit tests (`uv run pytest`, no database) and GitHub Actions running `ruff` and `pytest` on every push.
...
- (this commit) — docs: the October quarterly sequence rehearsed on 2026q2; the 1,701-company refresh explained as the pre-replay population
