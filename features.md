...
- **48-check verification suite** in `tools/verify_pit.sql`, run by `dera verify` (~90 s); checks 29–48 each name the 2026-09-04 review defect they guard. Plus 82 unit tests (`uv run pytest`, no database) and GitHub Actions running `ruff` and `pytest` on every push.
...
In reverse chronological order. The 2026-09-04 review landed as the six commits listed first; the S&P 500 membership and share-class work followed the same day.

- (this commit) — feat(reference,gold): dated S&P 500 membership from Wikipedia revisions; share classes decided by the filings and mapped from 10-K cover pages
