...
- **CIK succession.** `sec_reference.cik_succession` (211 ticker handoffs between registrants, from SEC's own file) lets the spine split a membership interval at a holding-company reorganisation: Apache's S&P 500 years are keyed to 6769 and APA Corp's to 1841666, Cigna's and WestRock's to the registrant that was filing. Five S&P 500 constituents of 2020-06-30 had no filer behind their CIK before; one remains (a ticker change at succession, Paramount Global → Paramount Skydance, which no handoff reveals). Check 59.
...
- **Dated S&P 500 membership, and gold reads it.** `sec_reference.index_membership` is replayed from 214 monthly Wikipedia captures since 2008 (840 companies ever in the index against 503 today; 25 pre-2010 tickers unresolved and listed, none with facts in DERA), with GICS as of the interval. The two `tradable_financials` matviews carry `index_name`, `index_is_asof` and as-of GICS per fact, and `peer_stats` scores only facts whose company was a constituent when the fact became actionable, so a FY2012 cross-section is 2012's index. `universe_at('sp500', date)` (508 securities on 2015-06-30) and `index_members('SP500', date)` expose it; the FY2012 peer panel holds 476 S&P 500 companies, FY2024 499. The S&P 400 and 600 followed on 2026-09-05.
...
- (this commit) — feat(gold): a zero total_debt from a debt-free balance-sheet face; pre_silver keeps the statements' layout
