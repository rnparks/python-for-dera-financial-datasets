...
- **Debt coverage, the remainder.** 148 S&P 400/600 members of 2024-12-31 file no debt-like balance line under any namespace, and 130 of them no interest-expense line either: almost certainly debt-free, but a zero from absence is an inference the layer has never made, so `total_debt` stays NULL for them until that call is made. 26 members are refused by the custom-line guard, and about 60 non-financials file instrument lines the variant does not apply to. Brokers and lenders (SIC 61–62) are excluded from the sum: a third carry custom lines.
...
- (this commit) — feat(gold): total_debt sums the instrument lines of a bank, insurer or REIT, guarded against custom lines; five industry revenue rows; every tag walk in dollars
